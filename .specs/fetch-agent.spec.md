...
## 目标
从配置的 RSS 源和 HTML 源抓取 AI 相关文章，去重、过滤，输出结构化文章列表。

## 感知 (Perceive)
- 输入：
  - `.pipeline/state.json` — 获取运行参数（hours_back）
  - `src/config.ts` — RSS_SOURCES 列表 + HTML_SOURCES 列表
...
- 决策4：HTML 源失败不阻塞管线，降级为空数组继续执行

## 行动 (Act)
- 输出：写入 `.pipeline/articles-raw.json`
- 调用：
  - `fetchAllFeeds()` — 抓取所有 RSS 源
  - `scrapeAllHtmlFeeds()` — 串行爬取所有 HTML 源（含 1s 延迟避免反爬）
  - `filterRecentArticles()` — 按时间过滤
  - `deduplicateArticles()` — 去重
  - `filterAIArticles()` — AI 关键词过滤（AI 源直接放行）
...
## 验收标准
- [ ] 成功抓取所有可用 RSS 源和 HTML 源（部分失败也可接受）
